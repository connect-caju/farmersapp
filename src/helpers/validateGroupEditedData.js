...
const validateGroupEditedData = (
  ...
  if (dataToBeUpdated === "groupType" && resourceName === "Group") {
    ...
    const retrievedOldGroupGoals =
      oldGroupGoals?.length > 0 ? oldGroupGoals : []
    ...
    if (
      ...
      setErrors({
        ...
        groupGoals:
          "A  finalidade do grupo actual não deve ser igual à anterior.",
      })
      return false
    }
  ...
  if (dataToBeUpdated === "groupIdentity" && resourceName === "Group") {
    const retrievedGroupAffiliationYear = groupAffiliationYear
      ? Number(parseInt(groupAffiliationYear))
      : 0
    const retrievedGroupLegalStatus = groupLegalStatus?.trim()
    const retrievedGroupNuit = groupNuit ? Number(parseInt(groupNuit)) : 0
    const retrievedGroupCreationYear = groupCreationYear
      ? Number(parseInt(groupCreationYear))
      : 0
    const retrievedGroupOperatingLicence = groupOperatingLicence.trim()

    const retrievedOldGroupAffiliationYear = oldGroupAffiliationYear
      ? Number(parseInt(oldGroupAffiliationYear))
      : 0
    const retrievedOldGroupLegalStatus = oldGroupLegalStatus?.trim()
    const retrievedOldGroupNuit = oldGroupNuit
      ? Number(parseInt(oldGroupNuit))
      : 0
    const retrievedOldGroupCreationYear = oldGroupCreationYear
    ...
    if (
      ...
      setErrors({
        ...errors,
        groupLegalStatus:
          "A situação legal actual não deve ser igual à anterior.",
        groupAffiliationYear:
          "O ano de legalização actual não deve ser igual ao anterior.",
        groupCreationYear:
          "O ano de criação actual não deve ser igual ao anterior.",
        groupNuit: "O NUIT actual não deve ser igual ao anterior.",
        groupOperatingLicence:
          "O alvará actual não deve ser igual ao anterior.",
      })
  ...
  if (dataToBeUpdated === "groupMembers" && resourceName === "Group") {
    ...
    const retrievedIsGroupInactive = isGroupInactive
      ? Boolean(isGroupInactive)
      : false
    const retrievedIsOldGroupActive = isOldGroupActive
      ? Boolean(isOldGroupActive)
      : isOldGroupActive
    const retrievedIsOldGroupInactive = isOldGroupInactive
      ? Boolean(isOldGroupInactive)
      : isOldGroupInactive

    const retrievedGroupMembersNumber = groupMembersNumber
      ? Number(parseInt(groupMembersNumber))
      : 0
    const retrievedGroupWomenNumber = groupWomenNumber
      ? Number(parseInt(groupWomenNumber))
      : 0

    const retrievedOldGroupMembersNumber = oldGroupMembersNumber
      ? Number(parseInt(oldGroupMembersNumber))
      : 0
    const retrievedOldGroupWomenNumber = oldGroupWomenNumber
      ? Number(parseInt(oldGroupWomenNumber))
      : 0

    if (
      retrievedIsGroupActive?.toString() ===
        retrievedIsOldGroupActive?.toString() &&
      retrievedIsGroupInactive?.toString() ===
        retrievedIsOldGroupInactive?.toString() &&
      ...
      setErrors({
        ...errors,
        operationalStatus:
          "O estado de funciomento actual não deve ser igual ao do anterior.",
        groupWomenNumber:
          "O total das mulheres actual não deve ser igual ao anterior.",
        groupMembersNumber:
          "O total dos membros actual não deve ser igual ao do anterior.",
      })
      return false
    }
    ...
    return {
      operationalStatus: retrievedIsGroupActive
        ? retrievedIsGroupActive
        : false,
