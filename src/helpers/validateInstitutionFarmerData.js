...
const validateInstitutionFarmerData = (
  ...
  const retrievedInstitutionManagerName = capitalize(
    institutionManagerName?.trim(),
  )
  const retrievedInstitutionManagerPhone = Number(
    parseInt(institutionManagerPhone),
  )
  ...
  if (
    (!retrievedIsInstitutionPrivate && !retrievedIsInstitutionPublic) ||
    (retrievedIsInstitutionPrivate && retrievedIsInstitutionPublic)
  ) {
    setErrors({
      ...errors,
  ...
  if (!retrievedInstitutionType) {
    setErrors({ ...errors, institutionType: "Indica tipo de instituição ." })
    return false
  }

  if (!retrievedInstitutionName) {
    setErrors({ ...errors, institutionName: "Indica nome da instituição." })
    return false
  }
  ...
  if (
    ...
    retrievedInstitutionManagerName?.split(" ").length <= 1
  ) {
    setErrors({ ...errors, institutionManagerName: "Indica nome completo" })
    return false
  }

  if (
    retrievedInstitutionManagerPhone !== 0 &&
  ...
  const farmerData = {
    ...
    manager: {
      ...
      phone: retrievedInstitutionManagerPhone
        ? parseInt(retrievedInstitutionManagerPhone)
        : 0,
    },
