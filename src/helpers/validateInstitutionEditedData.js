...
const validateInstitutionEditedData = (
  ...
  if (
    ...
    const retrievedInstitutionManagerName = institutionManagerName
      ? capitalize(institutionManagerName?.trim())
      : ""
    const retrievedInstitutionManagerPhone = institutionManagerPhone
      ? Number(parseInt(institutionManagerPhone))
      : 0

    const retrievedOldInstitutionManagerName = oldInstitutionManagerName
      ? capitalize(oldInstitutionManagerName?.trim())
      : ""
    const retrievedOldInstitutionManagerPhone = oldInstitutionManagerPhone
      ? Number(parseInt(oldInstitutionManagerPhone))
      : 0

    if (
      retrievedInstitutionManagerName === retrievedOldInstitutionManagerName &&
      retrievedInstitutionManagerPhone === retrievedOldInstitutionManagerPhone
    ) {
      setErrors({
        ...errors,
        institutionManagerName:
          "O nome do responsável actual não deve ser igual ao do anterior.",
        institutionManagerPhone:
          "O Número de telefone actual não deve ser igual ao anterior.",
      })
      return false
    }

    if (
      !retrievedInstitutionManagerName &&
      retrievedOldInstitutionManagerName
    ) {
      setErrors({
        ...errors,
    ...
    if (
      ...
      retrievedOldInstitutionManagerPhone
    ) {
      setErrors({
        ...errors,
        institutionManagerPhone:
          "Indica um novo número ou mantenha o anterior.",
      })
      return false
    }
  ...
  if (
    ...
    const retrievedInstitutionNuit = institutionNuit
      ? Number(parseInt(institutionNuit))
      : 0
    const retrievedInstitutionLicence = institutionLicence
      ? institutionLicence?.trim()
      : ""

    const retrievedOldInstitutionNuit = oldInstitutionNuit
      ? Number(parseInt(oldInstitutionNuit))
      : 0
    const retrievedOldInstitutionLicence = oldInstitutionLicence
      ? oldInstitutionLicence.trim()
      : ""
    ...
    if (
      ...
      setErrors({ ...errors, institutionNuit: "NUIT inválido." })
      return false
    }
